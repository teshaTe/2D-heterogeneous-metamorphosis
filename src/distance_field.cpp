...
void DistanceField::inverse_mapping_result(float *res, std::vector<float> SDF, float u_shifted, float v_shifted, cv::Mat affine_m)
{
    ...
    {
#ifdef USE_OPENMP
#pragma omp target data map(to: data[0:size], x, y) map(from:res)
{
#pragma omp target
       {
#endif
    ...
    else if( static_cast<int>(x) >= (im_h-1) && static_cast<int>(y) < (im_h-1) && x >= 0.0f && y >= 0.0f )
    {
        float FF1, FF2, aver;
#ifdef USE_OPENMP
#pragma omp target data map(from:FF1, FF2) map(to: data[0:size], y)
{
#pragma omp target
        {
#endif
        ...
        *res = get_massive_val(data, im_w-2, static_cast<int>(y), im_w-1) + aver;
        ...
        //*res = SDF[im_w-2 + static_cast<int>(y)*(im_w-1)] - std::sqrt( x*x / ( (im_w-1) * (im_w-1) ));
    }
    ...
    else if( static_cast<int>(x) < (im_w-1) && static_cast<int>(y) >= (im_h-1) && x >= 0.0f && y >= 0.0f )
    {
        float FF1, FF2, aver;
#ifdef USE_OPENMP
#pragma omp target data map(from:FF1, FF2) map(to: data[0:size], x)
{
#pragma omp target
       {
#endif
...
#pragma omp target
        ...
        *res = get_massive_val(data, static_cast<int>(x), 0, im_w-1) + aver;
        //*res = SDF[static_cast<int>(x)] - std::sqrt( y*y / ( (im_h-1) * (im_h-1) ));
    }
...
#pragma omp target
        ...
        *res = get_massive_val(data, 0, static_cast<int>(y), im_w-1) + aver;
        //*res = SDF[static_cast<int>(y)*(im_w-1)] - std::sqrt( x*x / ( (im_w-1) * (im_w-1) ));
    }
    ...
    else if( static_cast<int>(x) >= (im_w-1) && x >= 0.0f && y < 0.0f)
    {
        float FF1, FF2, aver;
#ifdef USE_OPENMP
#pragma omp target data map(from:FF1, FF2) map(to: data[0:size])
{
#pragma omp target
       {
#endif
        ...
        *res = get_massive_val(data, im_w-2, 0, im_w-1) + aver;
        //*res = SDF[im_w-2] - std::sqrt( y*y / ( (im_h-1) * (im_h-1) ) + x*x / ( (im_w-1) * (im_w-1)) );
    }
...
#pragma omp target
        ...
        *res = get_massive_val(data, im_w-2, im_h-2, im_w-1) + aver;
        //*res = SDF[im_w-2+(im_h-2)*(im_w-1)]- std::sqrt( y*y / ( (im_h-1) * (im_h-1) ) + x*x / ( (im_w-1) * (im_w-1)) );
    }
...
#pragma omp target
        ...
        *res = get_massive_val(data, 0, im_h-2, im_w-1) + aver;
        //*res = SDF[(im_h-2)*(im_h-1)] - std::sqrt( y*y / ( (im_h-1) * (im_h-1) ) + x*x / ( (im_w-1) * (im_w-1)) );
    }
...
#pragma omp target
        ...
        *res = get_massive_val(data, im_w-2, im_h-2, im_w-1) + aver;
        //*res = SDF[0] - std::sqrt( y*y / ( (im_h-1) * (im_h-1) ) + x*x / ( (im_w-1) * (im_w-1)) );
    }
...
void DistanceField::inverse_mapping_result(float *res, std::vector<float> SDF, float u_shifted, float v_shifted, cv::Mat affine_m, cv::Mat affine_rot_m)
{
    cv::invertAffineTransform(affine_m, inv_affine_m);
    cv::invertAffineTransform(affine_rot_m, inv_affine_rot_m);
    ...
    float r_x = (inv_affine_rot_m.at<double>(0, 0) * point.x +
                 inv_affine_rot_m.at<double>(0, 1) * point.y +
                 inv_affine_rot_m.at<double>(0, 2) * point.z);

    float r_y = (inv_affine_rot_m.at<double>(1, 0) * point.x +
                 inv_affine_rot_m.at<double>(1, 1) * point.y +
                 inv_affine_rot_m.at<double>(1, 2) * point.z);

    cv::Point3d point_sh = cv::Point3d(r_x, r_y, 1.0);

    float x = (inv_affine_m.at<double>(0, 0) * point_sh.x +
               inv_affine_m.at<double>(0, 1) * point_sh.y +
               inv_affine_m.at<double>(0, 2) * point_sh.z)+1.0f;

    float y = (inv_affine_m.at<double>(1, 0) * point_sh.x +
               inv_affine_m.at<double>(1, 1) * point_sh.y +
               inv_affine_m.at<double>(1, 2) * point_sh.z)+1.0f;
